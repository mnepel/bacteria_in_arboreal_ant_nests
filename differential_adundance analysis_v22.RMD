...
DropRareSpecies <- function(Ps_obj = physeq.ansys_pairwise, abundance = 0.5,  preval = 0.05) { # , prevalence = 0.1
    prevdf <- apply(
      X = otu_table(Ps_obj),
  ...
  # Add taxonomy and total read counts to this data.frame
    ...
    # keepTaxa <-
    #   row.names(prevdf_RareTax_filt)[(prevdf_RareTax_filt$Abundance >= abundanceThreshold)]
  ...
  if (OTU_labels) {
    p <- p + geom_label_repel(
      ...
      # data = ALDEx2plot[ALDEx2plot$Significance == "Pass" & 
                          # ALDEx2plot$baseMean > prevalence, ], # if including a prevalence threshold
      # nudge_x = 0.4,
      colour = "#4a4a4a",
      label.size = NA, 
      alpha = 0.75, 
...
if (dset == "EP"){
  ...
  (physeq.Ep.mean.abs.alf <- subset_samples(physeq.Ep.mean.abs, Ant == "alf"))
  (physeq.YEp.abs.alf <- merge_phyloseq(physeq.YCp.abs, physeq.Ep.mean.abs.alf))
  # (physeq.YEp.abs <- merge_phyloseq(physeq.YCp.abs, physeq.Ep.mean.abs))
  # (physeq.YEp.abs.alf <- subset_samples(physeq.YEp.abs, Ant == "alf"))
  (target.physeq <- physeq.YEp.abs.alf)
} else if (dset == "YEPcon") {
  (physeq.Ep.mean.abs.con <- subset_samples(physeq.Ep.mean.abs, Ant == "con"))
  (physeq.YEp.abs.con <- merge_phyloseq(physeq.YCp.abs, physeq.Ep.mean.abs.con))
  # (physeq.YEp.abs <- merge_phyloseq(physeq.YCp.abs, physeq.Ep.mean.abs))
  # (physeq.YEp.abs.con <- subset_samples(physeq.YEp.abs, Ant == "con"))
  (target.physeq <- physeq.YEp.abs.con)
}
```
...
# # family level
# (target.physeq_glom.fam <- tax_glom(target.physeq,
#                              "Family",
#                              NArm = TRUE))
#   (tax_table(target.physeq_glom.fam) <- tax_table(target.physeq_glom.fam)[,1:5])
# 
# # genus level
# (target.physeq_glom.genus <- tax_glom(target.physeq,
#                              "Genus",
#                              NArm = TRUE))
#   tax_table(target.physeq_glom.genus) <- tax_table(target.physeq_glom.genus)[,1:6]
#   head(tax_table(target.physeq_glom.genus))
```

## define physeq for ALDEx2 analyses
```{r}
physeq.ansys <- target.physeq_glom.order #_glom.genus # target.physeq
tax.level <-  "order" # "otu" # "genus" # order

# one line need to be adapted in the following script depending on tax. level
...
# (depicted_class <- medians[medians$median > 0.002, ]$Class) #

# # define Rare_tax based on the Classes displayed in order stacked bar graphs
# (Rare_tax <- unique(physeq.ansys_glom_rel_DF$Class[physeq.ansys_glom_rel_DF$Class %!in% c("c110.Actinobacteria", "c122.Bacteroidia", "c166.Bacilli", "c167.Clostridia", "c200.Alphaproteobacteria", "c201.Gammaproteobacteria", "c212.Verrucomicrobiae")]))
...
significance <- 0.05 # significant threshold
...
} else if ((dset == "FYP") | (dset == "YEP") | (dset == "YEPalf") | (dset == "YEPcon") | (dset == "FEP")) {
physeq.ansys %>% 
  sample_data() %>% 
  .$Type %>%
...
# for (j in seq(0.5,0.5,0.1)) { # seq(0.2,0.6,0.1)
  abundance <- 0.5 # j 0.5 = order.level ; 0.7 = EP.otu # taxa must be present in X number of samples (0.5 = 50%) 
  ...
  #  Relevel
  # sample_data(physeq.ansys_pairwise_s)$Ant %<>% fct_relevel(., "alf")
...
if (tax.level == "order"){
...
} else if ((tax.level == "otu") | (tax.level == "genus")){
  ...
  # # if only prevalent (meanRelAb > prevalence) taxa should be exported
  # ALDEx2plot_pairwise_results <- ALDEx2plot_pairwise_results[(ALDEx2plot_pairwise_results$baseMean > prevalence),]
  ...
  write.table(ALDEx2plot_pairwise_results, 
              paste(RESULTS_DIR,"/ALDEx2.lap.", dset, ".",tax.level,".ab", abundance, ".prev", preval,
...
```{r plot ALDEx2, cache=T, results = 'asis', fig.width=8, fig.height=7}
  p1 <- GGPlotALDExTax.Class.prev(ALDEx2plot_pairwise.prev, OTU_labels = TRUE, sig_level = significance, prev = prevalence) +
    ggtitle(paste("lap.", dset, ".", tax.level,".ab", abundance, ".prev", prevalence, sep = "")) +
    geom_text(
